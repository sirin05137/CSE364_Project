...
public class milestone3 {
    ...
    public static void main(String[] args) throws IOException{
        ...
        for(Map.Entry<String,ArrayList<String>> Iter : movie_data_map.entrySet()){
            if(Iter.getValue().get(0).equals(args[0])){
        ...
        for(Map.Entry<String,HashMap<String,Integer>> Iter : rating_data_map.entrySet()){
            double mean = get_average_rating(Iter.getValue());
            double distance = get_distance_of_vector(Iter.getValue(),mean);
            if(distance!=0) {
                BigDecimal Z = new BigDecimal("0.0");

                for (Map.Entry<String, Integer> Iter2 : Iter.getValue().entrySet()) {
                    if (rating_data_map.get(id_of_input).containsKey(Iter2.getKey())) {
                        BigDecimal X_i = new BigDecimal(String.valueOf(Iter2.getValue()));
                        BigDecimal X_mean = new BigDecimal(String.valueOf(mean));
                        BigDecimal Y_i = new BigDecimal(String.valueOf(rating_data_map.get(id_of_input).get(Iter2.getKey())));
                ...
                Similarity_of_movie inner_class = new Similarity_of_movie(Iter.getKey(), similarity.doubleValue());
